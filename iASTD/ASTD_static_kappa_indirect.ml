...
let extract_transition_labels astd = 
  ASTD_functions.remove_duplicates_from_left (List.map (ASTD_transition.get_label) (ASTD_astd.get_sub_transitions astd))


 
let rec get_non_kappa_in_arrow_list arrows var_met path =
  match arrows with
  | arrow::t -> 
    let variable_names = ASTD_transition.get_var_names_in_parameters_of_transition (ASTD_arrow.get_transition arrow) in
      if (is_included var_met variable_names) then
        (get_non_kappa_in_arrow_list t var_met path)
      else (
        debug (ASTD_transition.get_label (ASTD_arrow.get_transition arrow));
        (
          arrow,
          path,
          remove variable_names var_met
        )
        ::
        (get_non_kappa_in_arrow_list t var_met path)
      )
...
let rec get_var_name_from_vect vect = match vect with
  |(var_name,term)::tail-> var_name::(get_var_name_from_vect tail)
  |[]->[]


let rec mix_with vect var_met = ASTD_functions.remove_duplicates_from_left (var_met@(get_var_name_from_vect vect))

let rec extract_non_kappa_direct removed (var_met : ASTD_variable.name list) call_path path astd  = 
begin debug ("extract non kappa : "^(string_of_int (List.length var_met))^" var met in"^(ASTD_astd.get_name astd));
match astd with
  |ASTD_astd.Automata (name,attributes,optional_code,sub_astd,arrows,s_final,d_final,init) -> 
  ...
  |ASTD_astd.Sequence (name,attributes,optional_code,left,right) -> 
  ...
  |ASTD_astd.Choice (name,attributes,optional_code,left,right) -> 
    (extract_non_kappa_direct removed var_met call_path (path@[name]) left)@(extract_non_kappa_direct removed var_met call_path (path@[name]) right)

  |ASTD_astd.Kleene (name,attributes,optional_code,sub) -> 
    (extract_non_kappa_direct removed var_met call_path (path@[name]) sub)

    
  |ASTD_astd.Synchronisation (name,delta,attributes,optional_code,left,right) -> 
    (extract_non_kappa_direct removed var_met call_path (path@[name]) left)@(extract_non_kappa_direct removed var_met call_path (path@[name]) right)

  |ASTD_astd.Guard (name,attributes,optional_code,pred,sub) ->
    (extract_non_kappa_direct removed var_met call_path (path@[name]) sub)

  |ASTD_astd.QChoice (name,var,val_list,attributes,optional_code,dep,sub) ->
    let variable_name = ASTD_variable.get_name var in
      debug ("var in qchoice :" ^ (ASTD_variable.get_name var));
      (extract_non_kappa_direct (variable_name::removed) var_met call_path (path@[name]) sub)

  |ASTD_astd.QSynchronisation (name,var,val_list,delta,attributes,optional_code,opt,sub )-> 
    let variable_name = ASTD_variable.get_name var in
      debug ("var in qchoice :"^(ASTD_variable.get_name var));
      (extract_non_kappa_direct removed (variable_name::var_met) call_path (path@[name]) sub)
...
match astd with
  ...
  |ASTD_astd.Kleene (name,attributes,optional_code,sub_astd) -> 
    (extract_df var_met wait call_path (path@[name]) sub_astd)
    
  |ASTD_astd.Synchronisation (name,delta,attributes,optional_code,left,right) -> 
    if is_included (ASTD_functions.intersection (extract_transition_labels left) (extract_transition_labels right)) delta
  ...
  |ASTD_astd.QChoice (name,var,val_list,attributes,optional_code,dep,sub) ->
    let var_name = ASTD_variable.get_name var in
    if wait
      then extract_df var_met wait call_path (path@[name]) sub
      else begin debug ("found df for "^var_name); List.iter debug var_met ;
       ((path@[name]),var_met,var_name,[],[])::(extract_df var_met wait call_path (path@[name]) sub) end
  ...
  |ASTD_astd.QSynchronisation (name,var,val_list,delta,attributes,optional_code,opt,sub )-> 
    ...
    if is_included (extract_transition_labels sub) delta
      ...
      extract_df (var_name::var_met) false call_path (path@[name]) sub end
...
let rec extract_unessential_var (opti_list : ASTD_optimisation.optimisation list) label = match opti_list with
...
let rec find_prod arrows init_name init (opti_list : ASTD_optimisation.optimisation list) var_list sub_produced= match arrows with
  |arrow::t->
    let trans=ASTD_arrow.get_transition arrow in
    let (essential_var) = (remove (extract_unessential_var opti_list (ASTD_transition.get_label trans)) var_list) in
      if (ASTD_arrow.get_from arrow)=init_name then
        let variable_names = ASTD_transition.get_var_names_in_parameters_of_transition trans in
          if ((ASTD_astd.is_init_final init [])!="false")||(not (ASTD_arrow.get_from_final_state arrow))
              && (is_included essential_var variable_names) then
            let (prod,produced)=(find_prod t init_name init opti_list var_list sub_produced) in
              ((ASTD_transition.get_label trans)::prod,produced)
          else if sub_produced then
            find_prod t init_name init opti_list var_list sub_produced
          else 
            ([],false)
      else 
        find_prod t init_name init opti_list var_list sub_produced
...
let rec get_from_sub name arrows = match arrows with
  |(ASTD_arrow.From_sub(from,dest,through,trans,pred,final,optional_action))::t->
    if through=name
      then (ASTD_arrow.From_sub(from,dest,through,trans,pred,final,optional_action))::(get_from_sub name t)
      else get_from_sub name t
  |arrow::t-> get_from_sub name t
  |[]->[]

let rec get_to_sub name arrows = match arrows with
  |(ASTD_arrow.To_sub(from,dest,through,trans,pred,final,optional_action))::t->
    if through=name
      then (ASTD_arrow.To_sub(from,dest,through,trans,pred,final,optional_action))::(get_to_sub name t)
...
let rec get_producers var_met var call_path (opti_list : ASTD_optimisation.optimisation list) from_sub astd=match astd with
  |ASTD_astd.Automata (name,attributes,optional_code,sub_astd,arrows,s_final,d_final,init) ->
  ...
  |ASTD_astd.QChoice (name,var,attributes,optional_code,val_list,dep,sub) ->
    get_producers var_met (ASTD_variable.get_name var) call_path opti_list [] sub
...
let rec find_user_astd astd path var_to_find = 
  ...
  then failwith "mistake of matching in find_user_astd"
    ...
    in if sub_path=[]
      then failwith (" find_user_astd : before end, not found var "^(List.hd var_to_find)^" in "^(ASTD_astd.get_name astd))
      else begin debug (" find var "^(List.hd var_to_find)^" in astd "^(ASTD_astd.get_name astd)^" with next "^(List.hd sub_path));
        match astd with
        |ASTD_astd.Automata (name,attributes,optional_code,sub_astd,arrows,s_final,d_final,init) ->
        ...
        |ASTD_astd.QSynchronisation (name,var,val_list,delta,attributes,optional_code,opt,sub )-> begin 
          let var_name = ASTD_variable.get_name var in
          if List.mem var_name var_to_find
          then if (remove [var_name] var_to_find)=[]
            then begin print_endline (ASTD_astd.get_name sub);sub end
            else begin find_user_astd sub sub_path (remove [var_name] var_to_find) end
...
let rec get_consummers astd call_path f_sub t_sub wait =match astd with
  ...
  |ASTD_astd.Guard (name,attributes,optional_code,pred,sub) -> get_consummers sub call_path [] [] wait

  |ASTD_astd.QChoice (name,var,val_list,attributes,optional_code,dep,sub) -> get_consummers sub call_path [] [] wait

  |ASTD_astd.QSynchronisation (name,var,val_list,delta,attributes,optional_code,opt,sub )-> get_consummers sub call_path [] [] wait
...
let rec is_always_produced astd prod call_path from_sub = match astd with
  ...
  |ASTD_astd.Sequence (name,attributes,optional_code,left,right) -> if (ASTD_astd.is_init_final right [])!="false"
  ...
  |ASTD_astd.Kleene (name,attributes,optional_code,sub) -> is_always_produced sub prod call_path []

  |ASTD_astd.Synchronisation (name,delta,attributes,optional_code,left,right) -> 
    (is_always_produced left prod call_path [])&&(is_always_produced right prod call_path [])

  |ASTD_astd.Guard (name,attributes,optional_code,pred,sub) -> is_always_produced sub prod call_path []

  |ASTD_astd.QChoice (name,var,val_list,attributes,optional_code,dep,sub) -> is_always_produced sub prod call_path []

  |ASTD_astd.QSynchronisation (name,var,val_list,delta,attributes,optional_code,opt,sub )-> is_always_produced sub prod call_path []
...
let rec get_first_trans astd fst need met used trans= 
  let (name,attributes,optional_code,sub_astd,arrows,s_final,d_final,init)= ASTD_astd.get_data_automata astd
  in if (need=[init])||(need=[])
    ...
    in begin debug ("first_trans_from "^b);
      ...
      else let sub= ASTD_functions.remove_duplicates_from_left (List.map (ASTD_arrow.get_from) (extract_with_dest_from_list (get_from_sub init arrows) met))
        in if sub!=[]
          then begin debug "first_trans1";
            ...
            in let new_used= union (find_arrow_using_from_dest_origin_list 
              ...
              in get_first_trans 
                ...
                (remove
                  [b]
                  (union need (union  (ASTD_functions.intersection 
                      (List.map (ASTD_astd.get_name) sub_astd) 
                      (List.map (ASTD_arrow.get_from) (new_used)))  
                      (ASTD_functions.intersection 
...
match astd with
  ...
  |ASTD_astd.Kleene (name,attributes,optional_code,sub) -> is_produced sub prod label call_path []
  ...
  |ASTD_astd.Guard (name,attributes,optional_code,pred,sub) -> is_produced sub prod label call_path []

  |ASTD_astd.QChoice (name,var,val_list,attributes,optional_code,dep,sub) -> is_produced sub prod label call_path []

  |ASTD_astd.QSynchronisation (name,var,val_list,delta,attributes,optional_code,opt,sub )-> is_produced sub prod label call_path []
...
let extract_producers var_met var call_path (opti_list : ASTD_optimisation.optimisation list) from_sub astd = 
...
let rec update_producers_cons astd dep_list (opti_list : ASTD_optimisation.optimisation list) = match dep_list with
...
let match_user_creator used_var arrow var_met var prod cons_creator dep_path trans_path astd =
  ...
  and label=(ASTD_transition.get_label(ASTD_arrow.get_transition arrow))
  in if splitted 
    then begin debug ("var to match : "^var) ;let user_astd= find_user_astd astd trans_path (var::used_var)
...
let rec extract_dependency_path dependencies var_usable save_dep arrow astd trans_path var = match dependencies with
  |(path,var_met,dep_var,prod,cons)::t->
    ...
    if (match_user_creator var_met arrow var_met var prod cons path trans_path astd)&&(var=dep_var)
      then begin
        let missing=remove var_usable var_met
    ...
    with _ -> begin debug ("not_matched dependency with the label "^(ASTD_transition.get_label(ASTD_arrow.get_transition arrow))^" or var "^dep_var^" is not "^var);
        extract_dependency_path t var_usable save_dep arrow astd trans_path var
      end
    end
  |[]->let empty=([],[],var,[],[])
...
let rec find_one_opt arrow path astd dependencies (opt_list : ASTD_optimisation.optimisation list) var = match opt_list with
  ...
  |[]->begin debug ("find one opt / looking if "^(ASTD_transition.get_label (ASTD_arrow.get_transition arrow))^" is optimizable for var "^var);
        let (dep_dep,dep_list,optimizable)=extract_dependency_path 
                dependencies  
                (ASTD_transition.get_var_names_in_parameters_of_transition (ASTD_arrow.get_transition arrow)) 
                dependencies
                arrow
                astd
                path
...
let rec retrieve_optimisations astd dependencies (opt_list : ASTD_optimisation.optimisation list) dep_list non_kappa opt dep = match non_kappa with
...
let rec register_opt path (opt : ASTD_optimisation.optimisation) astd = 
  if (List.hd path)!=(ASTD_astd.get_name astd)
    ...
    in if sub_path=[]
      then failwith "register_opt end of path"
        ...
        |ASTD_astd.Sequence (name,attributes,optional_code,left,right) ->
          let left_name= ASTD_astd.get_name left
          in if left_name = (List.hd sub_path)
            then ASTD_astd.Sequence (name,attributes,optional_code,register_opt sub_path opt left ,right)
            else ASTD_astd.Sequence (name,attributes,optional_code,left,register_opt sub_path opt right)

        |ASTD_astd.Choice (name,attributes,optional_code,left,right) ->
          let left_name= ASTD_astd.get_name left
          in if left_name = (List.hd sub_path)
            then ASTD_astd.Choice (name,attributes,optional_code,register_opt sub_path opt left,right)
            else ASTD_astd.Choice (name,attributes,optional_code,left,register_opt sub_path opt right)

        |ASTD_astd.Kleene (name,attributes,optional_code,sub) ->
          ASTD_astd.Kleene (name,attributes,optional_code,register_opt sub_path opt sub)
        ...
        |ASTD_astd.Guard (name,attributes,optional_code,pred,sub) ->
          ASTD_astd.Guard (name,attributes,optional_code,pred,register_opt sub_path opt sub)

        |ASTD_astd.QChoice (name,var,val_list,attributes,optional_code,opt_list,sub) ->
        ...
        |ASTD_astd.QSynchronisation (name,var,val_list,delta,attributes,optional_code,opt_list,sub )->
          let (_,_,variable,_)=opt
          in if (ASTD_variable.get_name var) = variable
  ...
  then failwith "mistake of matching in register dep"
    ...
    in if sub_path=[]
      then if ASTD_astd.is_qchoice astd
        then let (name,var,val_list,attributes,optional_code,dep_list,sub)=ASTD_astd.get_data_qchoice astd
        ...
        |ASTD_astd.Sequence (name,attributes,optional_code,left,right) ->
          let left_name= ASTD_astd.get_name left
          in if left_name = (List.hd sub_path)
            then ASTD_astd.Sequence (name,attributes,optional_code,register_dep sub_path dep left ,right)
            else ASTD_astd.Sequence (name,attributes,optional_code,left,register_dep sub_path dep right)

        |ASTD_astd.Choice (name,attributes,optional_code,left,right) ->
          let left_name= ASTD_astd.get_name left
          in if left_name = (List.hd sub_path)
            then ASTD_astd.Choice (name,attributes,optional_code,register_dep sub_path dep left,right)
            else ASTD_astd.Choice (name,attributes,optional_code,left,register_dep sub_path dep right)

        |ASTD_astd.Kleene (name,attributes,optional_code,sub) ->
          ASTD_astd.Kleene (name,attributes,optional_code,register_dep sub_path dep sub)
        ...
        |ASTD_astd.Guard (name,attributes,optional_code,pred,sub) ->
          ASTD_astd.Guard (name,attributes,optional_code,pred,register_dep sub_path dep sub)

        |ASTD_astd.QChoice (name,var,val_list,attributes,optional_code,dep_list,sub) ->
