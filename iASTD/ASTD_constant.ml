...
let compare_constant c1 c2 = match (c1,c2) with
   ...
   (* | (FreeConst, _) | (_, FreeConst) -> 0 (* Free const, when compared with anything, always return equals*) *)
    | _ -> -1
...
let print_list = ASTD_functions.create_print_list print

let print_set_name n = print_string n

let print_set = ASTD_functions.create_print_set iteration_over_set print
...
let string_of_list = ASTD_functions.create_string_of_list string_of

let string_of_set_name n = n

let string_of_set = ASTD_functions.create_string_of_set fold_set string_of
...
let compare_val c1 c2 = match (c1,c2) with
    ...
    | FreeVal, _ | _, FreeVal -> 0 (* FreeVal is considered equal to anything *)
    | _ -> -1
...
let rec map_on_domain f domain =
  if domain = empty_dom then
    []
  else
    let (head, tail) = head_tail domain in 
      (f head)::(map_on_domain f tail)
