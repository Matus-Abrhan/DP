        ...
        def transform (self, spec, onto_types):
             ...
             for type, attribute in onto_types.items():
                 ...
                 for (event_label, variable) in re.findall(pattern, spec_):
                    ...
                    spec_ = spec_.replace(event_label + '(?' + variable + ':' + type + ')', target)
                    spec_ = spec_.replace(variable+".","")

                    files = [os.path.join(dp, f) for dp, dn, fns in os.walk(self.spec_path) for f in fns]

                    for f in files:
                        if self.GUARD_FILENAME in f.lower():
                            guard_f = open(f, "r")
                            content=str(guard_f.read()).replace(str(variable)+".","")
                            guard_f.close()
                            guard_f = open(f, "w+")
                        ...
                        if self.ACTION_FILENAME in f.lower(): 
                            action_f = open(f, "r")
                            content=str(action_f.read()).replace(str(variable)+".","")
                            action_f.close()
                            action_f = open(f, "w+")
...
if __name__ == "__main__":
        ...
        if len(sys.argv) < 2:
            ...
            if PATH:
               st = SpecTransformer()
               st.spec_path = os.path.abspath(os.path.join(sys.argv[1], os.pardir))
