...
let rec automatic_gestion_of_kappa_values env label (dep_list:dependency list) value= match dep_list with
  |(path,var_met,var,prod,cons)::t->let values= List.map (ASTD_environment.find_value_from_variable_name env) var_met
          in let opt=
